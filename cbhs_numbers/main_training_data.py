...
def main():
    ...
    digit = fh.get_digit()          # Digit to use in prompt.
    adjective = fh.get_adjective()  # Adjective to use in prompt.
    ...
    while running:
        ...
        if save_drawing:
            ...
            if canvas.pixels.any() and not canvas.pixels.all():
                fh.save_canvas(canvas.pixels, digit=digit, adjective=adjective)
                canvas.reset()                  # Reset the canvas.
                canvas.set_brush_radius()       # Randomize the brush radius.
                digit = fh.get_digit()          # Get a new digit.
                adjective = fh.get_adjective()  # Get a new adjective.
        ...
        if ENABLE_TEXT:
            draw_upper_text(screen, prompt_font, fh.construct_prompt(digit, adjective))
            draw_lower_text(screen, font)
            
        # If we are not drawing text to the screen, put the prompt in the window title.
        else:
            pg.display.set_caption(fh.construct_prompt(digit, adjective))
